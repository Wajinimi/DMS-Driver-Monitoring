...
# I want to test the full AI pipeline from Phase 1 through Phase 7.
...
def main():
    ...
    logger.info("Phase 7 live — logging trip %s. Press 'q' to quit.", trip_id)
    ...
    try:
        while streamer._running:
            ...
            if preview is not None:
                ...
                cv2.imshow("DMS Phase 7 — Analytics", shown)
        ...
        logger.info("Trip summary: %s", summary)

        week = analytics.get_last_7_days_summary()
        logger.info("Plot 1 preview (last 7 days):")
        for day in week:
            logger.info(
                "  %s %s: %s min",
                day["weekday"],
                day["date"],
                day["total_distraction_minutes"],
            )

        analytics.close()
        logger.info("I submitted %d clips and finished Phase 7 test.", clips_submitted)
